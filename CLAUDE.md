...
- 게시 **15일 뒤**부터는 ④ **1일 추세**(최근 하루 증가 g × 하루마다 줄어드는 비율 r)로 다음 **100만 단위**까지 며칠인지 계산하고
  ...
  화면은 15일 지난 영상을 100만 단위 모드로 보여 주고(그래프는 지금 구간의 100만 단위를 넘은 때(기록 전이면 첫 기록)부터, 가로축 [1일: 14일 뒤까지 | 1주: 8주 뒤까지], 15분·1시간 등 증가 막대는 없음), 순위 줄 탭 **[예측 조회수 | 100만 단위 돌파]** 중 100만 탭은 **게시 15일 지난** 조회수 90만 이상 영상의 다음 100만 넘는 때 목록 (④ 1일 추세로만 계산, 15일 전 영상은 넣지 않는다. 줄 오른쪽 아래 추이도 ④ 1주 증가 순위)
