...
- 게시 **3주 뒤**부터는 ④ 장기 추세로 다음 **100만 단위** 돌파를 예측하고 `ms` 로 고정 저장, 닿으면 `hit` 기록.
  장기 예측이라 **주 단위(1주 ~ 8주)**: 주마다 그때까지 넘을 확률(`p`), 처음 50% 넘는 주 = 유력 주차.
  화면은 3주 지난 영상을 100만 단위 모드로 보여 주고, 8주 안 확률 50% 이상인 영상은 상승 순위 옆 **"100만 단위 돌파 유력" 버튼**으로 모은다
- 조회수 예측기 화면 색은 **초록 계열**(사이트 나머지의 분홍과 다름). 변수는 theme.css 의 `#v-views{--vg…}`
