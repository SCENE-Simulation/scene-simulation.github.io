...
- `tools/share.js` — **공유 카드**(`window.SGShare`): 그 순간 기준으로 고정된 PNG 이미지를 브라우저 캔버스에서 그린다(서버 없음, 실시간 아님 — 디시 등에 올리는 용도).
  [공유하기]를 누르는 순간 클립보드 복사를 시작(ClipboardItem 에 Promise — 누른 순간이어야 허락됨), 실패하면 [클립보드에 복사] 버튼, 그래도 안 되면 [이미지 저장]으로 바뀜. 휴대폰은 [공유…]도. 지금은 조회수 예측기 영상 머리글의 [공유하기] 버튼(`SGShare.video`, 글자는 views.js `shareCard(v)` 가 만든다).
  유튜브 썸네일은 CORS 가 열려 있어 캔버스에 그릴 수 있다(maxres → hq → 수집 주소 순). 컬렉션 카드는 다음에 추가
