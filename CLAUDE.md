...
- `tools/share.js` — **공유 카드**(`window.SGShare`): 그 순간 기준으로 고정된 PNG 이미지를 브라우저 캔버스에서 그린다(서버 없음, 실시간 아님 — 디시 등에 올리는 용도).
  [공유 카드]를 누르는 순간 클립보드 복사를 시작(ClipboardItem 에 Promise — 누른 순간이어야 허락됨), 실패하면 [클립보드에 복사] 버튼, 그래도 안 되면 [이미지 저장]으로 바뀜. 휴대폰은 [공유…]도. 지금은 조회수 예측기 영상 머리글의 [공유 카드] 버튼(`SGShare.video`, 글자는 views.js `shareCard(v)` 가 만든다).
...
- **예측 방법은 ①②③ 세 가지** (색: ① 노랑 #ecd25b · ② 참여도 보라 #a080d0 · ③ 추세 곡선 민트 #6fe0b3 = 100만 단위 화면 색 `MSC`). **"게시 15일 뒤" 조건은 없다** (9/24 삭제).
  ...
  k 는 최근 7일 앞·뒤 반쪽에서 재고(0.6~2.5) 모자라면 K0 = 1.3. 24시간·7일·30일 예측(`METHODS` m3, 예측 시점까지의 기록만)과 다음 100만 단위 돌파(`msPlan`)가 같은 곡선. 9/24 전에 고정된 `pred` 의 ③ 은 옛 로그 곡선 값
