...
(function(root){
  ...
  var METHODS = [
    ...
    // ③ 추세 곡선법: 게시 t시간까지의 기록으로 최근 하루 증가 g 와 감쇠 지수 k 를 재고(daily), 하루 증가가 영상 나이에 반비례하는
    //    거듭제곱 (d0/(d0+i))^k 로 줄어든다고 보고 T 까지 이어 붙인다(dayProject). 다음 100만 단위 돌파 예측도 같은 곡선.
    //    최근 기록이 3시간은 있어야 한다. 과거 영상이 필요 없어 수집 초기에도 나온다
    { key: 'm3', point: function(v, t, T){
        if (t < 1) return null;
        var a = at(v, t, 1), P = daily(v, t);
        if (!(a > 0) || !P) return null;
        return Math.max(a, dayProject({ V: a, g: P.g, k: P.k, d0: P.d0 }, (T - t) / 24));
      } }
  ];
  // ③ 을 쓸 수 있으려면 게시 t시간 째여야 하는지 (기록 시작 뒤 3시간)
  function m3From(v){ return since(v) + 3; }
  ...
  // ---------- ③ 추세 곡선 (감쇠 추세): 24시간·7일·30일 예측(METHODS m3)과 다음 100만 단위까지 며칠 ----------
  ...
  // 을 넘은 순간 ③ 추세 곡선으로 구간 끝(M0+100만) 도달을 예측해 고정하고, 실제로 닿은 때와 비교한다 (수집기가 고정, 사이트가 채점).
