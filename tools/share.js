...
(function(){
  ...
  // ---------- 예측의 신 카드 (내 예측 기록 하나) — 화면 기록 카드와 같은 짜임 (9/24) ----------
  // d = { at, title, thumbs, hue, M: '1,000만 돌파', st: 'wait'|'done'|'void'|'gone', guess: 내 예측(글),
  //       my: { t: 내 예측 아랫줄, tone: 'fast'|'slow'|'eq'|'' }, mine: [[이름, 값] × 2],
  //       ai: { when, sub } | null (예측기 예상), done: { acc, grade, gc } | null,
  //       wait: { left, over, remain, nowgoal, prob, ptone: 'hi'|'mid'|'lo'|'' } | null }
  var OG = { p: '#a98bff', pt: '#cdb9ff', au: '#f0c75e', mint: '#6fe0b3', red: '#ff9a9a' };
  function drawOracle(d, im){
    ...
    function dot(cx, cy, col){ x.fillStyle = col; x.beginPath(); x.arc(cx, cy, 6, 0, Math.PI * 2); x.fill(); }
    // 예측기 예상 (민트 상자, 한 줄: 이름 · 때 ··· 아랫말)
    if (d.ai){
      rr(x, P, AY, AW, AH, 16); x.fillStyle = 'rgba(111,224,179,.07)'; x.fill(); x.strokeStyle = 'rgba(111,224,179,.35)'; x.lineWidth = 1.5; x.stroke();
      x.textBaseline = 'middle'; var ay = AY + AH / 2 + 1;
      dot(P + 28, ay, OG.mint);
      font(x, 800, 21); x.fillStyle = OG.mint; x.fillText('예측기 예상', P + 44, ay);
      var ax = P + 44 + x.measureText('예측기 예상').width + 18;
      font(x, 800, 26); x.fillStyle = C.ink; x.fillText(d.ai.when, ax, ay);
      ax += x.measureText(d.ai.when).width + 20;
      if (d.ai.sub){ font(x, 600, 20); x.fillStyle = C.ink2; x.textAlign = 'right'; x.fillText(fit(x, d.ai.sub, W - P - 26 - ax), W - P - 26, ay); x.textAlign = 'left'; }
    ...
    if (d.wait){
      var GAP = 14, bw3 = (AW - GAP * 2) / 3, w = d.wait;
