...
//  - 다음 100만 단위 돌파 예측을 고정 저장하고, 닿으면 닿은 시각을 적는다 (ms — 화면에서는 더 쓰지 않음)
//  - 영상이 100만 단위 구간의 세 지점(100만·50만·20만 남음)을 넘으면 구간 끝 도달 예측을 고정하고, 닿으면 적는다 (seg)
//
...
//                 ms:    [ { M: 100만 단위 목표, t: 예측한 때(h), v: 그때 조회수, g: 그때 하루 증가, k: 감쇠 지수(views-engine ③ 추세 곡선), r: 내일 하루 증가가 오늘의 몇 배,
...
//                                   g, k, r, src: 그때 ③ 추세 곡선, far?: 1(8주보다 멀거나 못 닿는다고 봄 → 참고) } ] } ] } ] }
...
async function main(){
  ...
  // 5) 다음 100만 단위 돌파 예측을 한 번 고정하고(기록이 2일 이상 있어야), 닿으면 닿은 때를 적는다
  for (const V of vids){
  ...
  // 5-2) 100만 단위 구간 채점: 지난 수집 값 → 이번 값 사이에 구간 지점(M0, M0+50만, M0+80만)을 넘었으면
  //      ③ 추세 곡선으로 구간 끝(M0+100만) 도달을 예측해 고정하고, 구간 끝을 넘었으면 닿은 때를 적는다.
